...
def get_existing_article_ids(index) -> Set[int]:
    ...
    for match in results.matches:
        if match.metadata and 'article_id' in match.metadata:
            article_ids.add(match.metadata['article_id'])
...
def delete_article_chunks(index, article_id: int):
    ...
    """
    # Delete vectors where metadata.article_id matches
    index.delete(filter={"article_id": article_id})
...
def main():
    try:
        ...
        articles = article_service.get_all_published_articles()  # Get all published articles
        ...
        # Get new article IDs
        new_article_ids = {article.id for article in articles}
        ...
        if articles_to_delete:
            ...
            for article_id in articles_to_delete:
                ...
                print(f"Deleted chunks for article {article_id}")
